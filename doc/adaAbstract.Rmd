...
dsBase %>% 
  select(UDBPStatus, ACR) %>% 
  na.omit() %>% 
  box_plot("UDBPStatus", "log(ACR)",
           "Urinary VDBP status",
           "log Urine ACR (mg/mmol)")
...
gee <- ds %>%
  ...
  mason::add_variables("covariates", 
                       c("VN", "ageBase", "Sex", "Ethnicity", "BMI", "fDM")) %>% 
  mason::construct() %>%
  mason::scrub()
```
...
gee %>% 
  ...
  dplyr::mutate(Yterms = factor(Yterms, 
                ...
                Xterms = factor(Xterms,
                                ...
                                ordered = TRUE)) %>% 
  arrange(Xterms) %>% 
  gee_plot(xlab = "Unit change for every unit increase in uVDBP")
```
